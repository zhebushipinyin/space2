...
visual.TextStim(win, text='您本试实验判断能力得分为：%s/100分' % judge_point, pos=(0, 1)).draw()
visual.TextStim(win, text='您本试实验运动能力得分为：%s/100分' % all_point, pos=(0, -1)).draw()
visual.TextStim(win, text='本次实验结束，双击屏幕或点击鼠标退出', pos=(-8, 4)).draw()
win.flip()
while sum(myMouse.getPressed(getTime=True)[0]) == 0:
    continue
win.close()
